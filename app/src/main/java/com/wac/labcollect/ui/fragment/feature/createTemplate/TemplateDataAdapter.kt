...
class TemplateDataAdapter(mDataset: MutableList<Pair<String, DataType>>) : DragDropSwipeAdapter<Pair<String, DataType>, TemplateDataAdapter.ViewHolder>(mDataset) {
    ...
    override fun onBindViewHolder(item: Pair<String, DataType>, viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.colName.setText(mutableDataSet[viewHolder.bindingAdapterPosition].first)

        viewHolder.colTypeWatcher.updatePosition(viewHolder.bindingAdapterPosition)
        viewHolder.colType.apply {
            setText(mutableDataSet[viewHolder.bindingAdapterPosition].second.type.textValue)
    ...
    inner class NameTextWatcher : TextWatcher {
        ...
        override fun onTextChanged(charSequence: CharSequence, i: Int, i2: Int, i3: Int) {
            mutableDataSet[position] = mutableDataSet[position].copy(first = charSequence.toString())
        }
    }
    ...
    inner class TypeTextWatcher : TextWatcher {
        ...
        override fun afterTextChanged(editable: Editable) {}
        override fun onTextChanged(charSequence: CharSequence, i: Int, i2: Int, i3: Int) {
            mutableDataSet[position] = mutableDataSet[position].copy(second = DataType(DataType.getTypeFromTextVal(charSequence.toString())))
        }
    }
